...
const HEADLINE = "ഏലങ്കുളം കാലാവസ്ഥാ വിശദ റിപ്പോർട്ട്";
// ----------------------------------------
...
function generateLongNewsMalayalam({ computed, imdAlert, airQuality }){
  ...
  if (computed.tempNow != null) {
    s.push(`ഇപ്പോൾ രേഖപ്പെടുത്തപ്പെട്ട താപനില ${toFixedSafe(computed.tempNow,1)}°C ആണ്; ഇത് കഴിഞ്ഞ കുറച്ചുമണിക്കൂറുകളിലെ ശരാശരിയുടെ അടിസ്ഥാനത്തിൽ വിലയിരുത്തുമ്പോൾ സമാന നിരപ്പിനേക്കാൾ  അല്പം ${computed.tempTrend>0.15 ? 'ഉയർന്ന ' : (computed.tempTrend < -0.15 ? 'താഴ്ന്ന' : 'സ്ഥിരമായ')} നിലപാടിലാണ് കാണപ്പെടുന്നത്.`);
  } else {
  ...
  s.push("ഇന്ന് പ്രദേശത്ത് ആകെ മേഘാവൃതമായി നിലനിൽക്കുന്നുവെന്ന് നിരീക്ഷണങ്ങൾ സൂചിപ്പിക്കുന്നു; മേഘാവൃതമായി പല ഭാഗങ്ങളിലും പരിമിതമായി കാണപ്പെടുന്നു.");

  // Paragraph 2: humidity and feel (2-3 sentences)
  if (computed.humidity != null) {
    s.push(`വായുവിലെ ശുദ്ധി നിർണ്ണായക ഘടകമായ ഹുമിഡിറ്റി നിലവിൽ ശ്രദ്ധിച്ചാൽ, നിലവിൽ  ${Math.round(computed.humidity)}% ആണ്; ഈ നില പ്രവണതയിൽ വരുന്ന ചെറിയ വ്യതിയാനങ്ങളോടെ പ്രാദേശികമായി വായു തണുത്തതോ ചൂടേറിയതയോ അനുഭവപ്പെടാം.`);
  } else {
    s.push("ഹ്യൂമിഡിറ്റി വിശകലനത്തിന് നിലവിലെ വിശ്വസ്തമായ ഡാറ്റ ലഭ്യമല്ല.");
  }
  s.push("മൊത്തത്തിൽ, അന്തരീക്ഷത്തിലെ ഈർപ്പ മാറ്റങ്ങൾ അന്തരീക്ഷ ദ്രവ സംയോജനത്തെ ബാധിച്ചേക്കാവുന്നവയാണ്.");
  ...
  if (computed.windSpeedMs != null) {
    ...
    s.push("ഈ വേഗതയിൽ കാറ്റ് വീശുന്നതിനാൽ, പ്രദേശിക തലത്തിൽ ശക്തമായ മഴയോ കടലിന്റെ നിലയിൽ വലിയ മാറ്റങ്ങളോ ഉണ്ടാകാനുള്ള സാധ്യത കുറഞ്ഞതാണ് എന്ന് വിലയിരുത്തുന്നു.");
  ...
  s.push("ദേശീയ നിരീക്ഷണ കേന്ദ്രങ്ങളിലെ ഡാറ്റ പ്രകാരം മേഘാവരണത്തിലെ മാറ്റങ്ങൾ സാധാരണഗതിയിൽ രേഖപ്പെടുത്തപ്പെട്ടുകൊണ്ടിരിക്കുന്നു.");
  ...
  if ([6,7,8,9].includes(month)) {
    s.push("മൂസൂൺ കാലയളവിലേക്ക് കടന്നതിനാൽ, ഒരുവശത്ത് വെയിലും മറുവശത്ത് ശക്തമായ മഴയും മാറിമാറി അനുഭവപ്പെടുന്ന സാഹചര്യങ്ങൾ പ്രതീക്ഷിക്കാം.");
  } else if ([10,11,12,1].includes(month)) {
    s.push("പ്രദേശം ശൈത്യകാലം എന്ന ഘട്ടത്തിലേക്കാണ് ഇപ്പോൾ കടക്കുന്നത്/കടന്നിരിക്കുന്നത്. രാത്രികളിൽ താപനില കുറയുന്നതിനാൽ അല്പം തണുപ്പ് അനുഭവപ്പെടുന്നു. പുലർച്ചെ സമയങ്ങളിൽ ചെറുതായി മഞ്ഞ്/മൂടൽമഞ്ഞ് രൂപപ്പെടാനും സാധ്യതയുണ്ട്.");
  } else {
    s.push("ഈ സമയത്ത് പ്രദേശത്തെ കാലാവസ്ഥയിൽ മിശ്രമായ ഘടകങ്ങൾ വ്യക്തമായി തുടരുന്നു. മേഘാവരണം ശക്തമായി അനുഭവപ്പെടുന്ന ഇടവേളകളോടൊപ്പം, ഇടയ്ക്കിടെ ചെറിയതോ മിതമായതോമായ മഴയും പെയ്യുന്ന സാഹചര്യമാണ് റിപ്പോർട്ടുകൾ വ്യക്തമാക്കുന്നത്.
");
  }

  // Paragraph 7: alerts and AQI (2-3 sentences)
  if (imdAlert && imdAlert.text) {
    s.push(`മലപ്പുറം ജില്ലക്ക് IMD ഔദ്യോഗിക അറിയിപ്പ്: ${imdAlert.text} (അവസാന അപ്‌ഡേറ്റ്: ${imdAlert.lastUpdated || 'ലഭ്യമല്ല'}). ഈ അറിയിപ്പ് മാന്യമായി റിപ്പോർട്ടിൽ ഉൾപ്പെടുത്തിയിട്ടുണ്ട്.`);
  } else {
  ...
  s.push("പ്രാദേശിക നിരീക്ഷണങ്ങൾക്കും സമൂഹത്തില്‍നിന്നുള്ള റിപ്പോർട്ടുകൾക്കും പ്രധാനപ്പെട്ട സ്ഥാനം ഉണ്ട്. താഴെ ലഭ്യമായ റിപ്പോർട്ടുകൾ ഉൾപ്പെടുത്തിയിട്ടുണ്ട്.");
  ...
  if (sentenceCount < 20) {
    const fillers = [
      "ലഭ്യമായിരിക്കുന്ന ഡാറ്റ ഇപ്പോഴും പൂർണ്ണമല്ലാത്തതിനാൽ, കാലാവസ്ഥാപരമായ ഏതെങ്കിലും വ്യതിയാനങ്ങൾ ഉറപ്പായി നിർണ്ണയിക്കാനുള്ള സാഹചര്യം നിലവിലില്ല.",
