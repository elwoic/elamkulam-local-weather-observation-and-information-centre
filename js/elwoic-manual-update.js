document.addEventListener("DOMContentLoaded", () => {
  ...
  const manualMetaEl = document.getElementById("alert-manual-meta");
  const preMetaEl    = document.getElementById("alert-pre-meta");
  const infoMetaEl   = document.getElementById("alert-info-meta");
  ...
  function formatMeta(blocks) {
    if (!blocks || blocks.length === 0) return "";
    const lastUpdated = Math.max(...blocks.map(b => b.created_at || 0));
    // If ANY currently-showing block in this section never expires, the
    // section itself has no fixed end — it stays up until manually revoked.
    const hasOpenEnded = blocks.some(b => !b.expires_at);
    const validUntilStr = hasOpenEnded
      ? "until revoked"
      : new Date(Math.max(...blocks.map(b => b.expires_at))).toLocaleString();
    const updatedStr = lastUpdated ? new Date(lastUpdated).toLocaleString() : "";
    return `Updated ${updatedStr} · Valid ${validUntilStr}`;
  }

  function renderStacked(el, metaEl, blocks, emptyText, color) {
    ...
    if (metaEl) metaEl.textContent = formatMeta(blocks);
  ...
  async function fetchLiveAlerts() {
    try {
      ...
      const manualResult = renderStacked(manualEl, manualMetaEl, manualBlocks, "No updates available at this time", "red");
      const preResult    = renderStacked(preEl, preMetaEl, preBlocks, "No updates scheduled", "#f1c40f");
      const infoResult   = renderStacked(infoEl, infoMetaEl, infoBlocks, "No general information", "");
