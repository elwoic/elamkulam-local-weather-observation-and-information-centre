...
function _wd_init() {
  ...
  function updateAll() {
    Promise.all([
      ...
      /* ── hero video (NEW) ──
         Only swap in a clip for dry daytime families we have footage for.
         Everything else (rain/storm/fog/night, or missing signals) keeps
         showing the existing gradient/FX layer untouched. */
      if (isDayFlag && DRY_DAYTIME_FAMILIES.indexOf(theme.family) !== -1) {
        var kc = ncStats.clear_sky_index_kc != null ? ncStats.clear_sky_index_kc : null;
        updateHeroVideo(skyConditionFromSignals(kc, cloudPct));
      } else {
